...
def load_arguments():
    ...
    parser.add_argument("--experiment_name", type=str, default="byt5-full-finetune",
    ...
    # Encoder freezing
    parser.add_argument("--freeze", type=str, default="none", choices=["none", "encoder"],
                        help="Freeze the encoder to only train the decoder.")
    ...
    parser.add_argument("--learning_rate",              type=float, default=3e-3,
                        help="3e-3 is safer; try 1e-3 to 5e-3.")
    parser.add_argument("--weight_decay",               type=float, default=0.01)
    parser.add_argument("--lr_scheduler_type",          type=str,   default="linear")
    parser.add_argument("--warmup_steps",               type=int,   default=500,
                        help="Original 5000 may be too many for small datasets.")
    parser.add_argument("--gradient_checkpointing",     action="store_true", default=False,
                        help="Enable to reduce GPU memory at the cost of speed.")

    # Logging / evaluation / saving
    parser.add_argument("--logging_steps", type=int, default=100,
                        help="Log loss every N steps. Keep well below total steps.")
    parser.add_argument("--eval_steps",    type=int, default=1000,
                        help="Run evaluation every N steps.")
    parser.add_argument("--save_steps",    type=int, default=1000,
                        help="Save checkpoint every N steps. Should align with eval_steps.")
    parser.add_argument("--save_total_limit", type=int, default=2,
                        help="Keep only the N most recent checkpoints.")
    parser.add_argument("--early_stopping_patience", type=int, default=5,
                        help="Stop if eval metric does not improve for N evaluations. "
                             "Set to 0 to disable.")
...
def main():
    ...
    if args.tokenization == "spaces" and smi_tokenizer_spaces is None:
        print("ERROR: --tokenization spaces requested but tokenization module not found.")
        sys.exit(-1)
    ...
    # ── Model & tokenizer ─────────────────────────────────────────────────────
    # Use os.chdir workaround for newer huggingface_hub versions on Windows
    ...
    model = T5ForConditionalGeneration.from_pretrained(
        ".", use_cache=not args.gradient_checkpointing, local_files_only=True
    )
    ...
    print(f"Train samples: {len(dataset['train'])}, Val samples: {len(dataset['val'])}")
    ...
    # Save all args to disk for reproducibility
    ...
    training_args = Seq2SeqTrainingArguments(
        ...
        fp16=False,  # Overflows with fp16 on T5
        ...
        # Logging — loss is written to trainer_state.json every logging_steps
